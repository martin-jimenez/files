...
if [ "$PAM_TYPE" != "close_session" ]; then
        ...
        channel=<n>
        host="$(hostname)"
        content="\"attachments\": [ { \"mrkdwn_in\": [\"text\", \"fallback\"], \"fallback\": \"SSH login: $PAM_USER connected to \`$host\`\", \"text\": \"SSH login to \`$host\`\", \"fields\": [ { \"title\": \"User\", \"valu>        text="SSH login: <b>$PAM_USER</b> on <b>$host</b> <b>$PAM_RHOST</b>"
